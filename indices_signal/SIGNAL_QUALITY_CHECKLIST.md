...
- [x] **Change of Character (ChoCH) gate** — `require_choch` flag in `SIGNAL_CONFIG` (default on); requires price to break above recent swing high (BUY) or below recent swing low (SELL) before signal fires; confirms momentum is moving in signal direction, not just indicators aligning; `choch_swing_lookback` (5 bars) and `choch_search_window` (20 bars) control sensitivity; shows "ChoCH confirmed" in Telegram reasons
